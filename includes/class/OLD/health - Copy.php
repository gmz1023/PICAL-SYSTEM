		...
		if($checker == 1)
		{
			...
			try  {$que->execute(); 
				while($row = $que->fetch(PDO::FETCH_ASSOC))
				{
					...
					if(
					(	$row['hubhealth'] >= 45 && $row['wealth'] >= 45) 
						&& 
						mt_rand(0,600) <= 2
						&&
						($wage >= 18 && $hage >= 18) 
...
{
		$time = $this->getTime();
		$sql = "UPDATE citizens SET status = {$status} WHERE cid = {$cid}";
...
/* Health Functions */
	...
	function kill($cid, $reason)
	{
		...
		$name = $this->getname($cid);
					$sql = "UPDATE citizens SET status = 0, spouse_id = 0 WHERE cid = :cid";
					$que = $this->db->prepare($sql);
					$que->bindParam(':cid', $cid);
					try { $que->execute(); 
